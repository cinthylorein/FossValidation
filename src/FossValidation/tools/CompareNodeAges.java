...
public class CompareNodeAges extends Runnable {
	final public Input<TreeFile> summaryTree = new Input<>("tree1", "true tree file with tree in NEXUS format");
	final public Input<List<TreeFile>> srcInput = new Input<>("trees", " Reference trees file, with n calibration  in NEXUS format", new ArrayList<>());
	    ...
	    for (int k = 0; k < speciesTree.size() && k < fossilTrees.size(); k++) {
	        ...
	        for (int i = 0; i < fossilNodes.length; i++) {
	            ...
	            if (!fossilNode.isLeaf() && !speciesNode.isLeaf()) {
	                ...
	                double difference = (fossilHeight - speciesHeight);
	...
	public static void main(String[] args) throws Exception {
		new Application(new CompareNodeAges(), "CompareNodeAges", args);
	}
